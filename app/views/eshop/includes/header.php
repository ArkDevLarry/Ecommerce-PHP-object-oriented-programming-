...
<head>
    ...
    <title><?=isset($data['title']) ? $data['title'] : 'Shop Online :: No 1 Shopping Site for Pro\'s'?></title>
    <meta name="description" content="">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <!-- Place favicon.ico in the root directory -->
    <link rel="shortcut icon" type="image/x-icon" href="<?=ASSET.FRNTND.'img/favicon.png'?>">
    <!-- CSS here -->
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/preloader.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/bootstrap.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/meanmenu.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/animate.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/owl-carousel.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/swiper-bundle.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/backtotop.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/ui-range-slider.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/magnific-popup.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/nice-select.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'flaticon/flaticon.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/font-awesome-pro.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/default.css'?>">
    <link rel="stylesheet" href="<?=ASSET.FRNTND.'css/style.css'?>">
</head>
                                ...
                                <div class="header__currency border-right">
                                    ...
                                    ob_start();
                                        if (isset($_POST['lang_name'])) {
                                            setcookie('lang',$_POST['lang_name'],time()+3600 * 24 * 5);
                                            show($_POST['lang_name']);
                                        }
                                        ob_end_clean();
                                    ?>
                                   <form method="POST">
                                        <select name="lang_name" onchange="submit()">
                                            <option value="en_US" <?=cookie_check('lang') == "en_US" ? 'selected' : ''?>>English</option>
                                            <option value="de_DE" <?=cookie_check('lang') == "de_DE" ? 'selected' : ''?>>Deutsch</option>
                                            <option value="fr_FR" <?=cookie_check('lang') == "fr_FR" ? 'selected' : ''?>>Français</option>
                                            <option value="es_US" <?=cookie_check('lang') == "es_US" ? 'selected' : ''?>>Espanol</option>
                                        </select>
                                   </form>
                                </div>
                            ...
                            <div class="header__info">
                                <div class="logo d-flex justify-content-center align-items-center">
                                    <a href="index.html" class="logo-image">
                                        <img src="<?=ASSET.FRNTND.'img/logo/kartplug.png'?>" alt="logo">
                                    </a>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-xl-4 col-lg-5 col-md-8 col-sm-8">
                            <div class="header-action">
                                <div class="block-userlink">
                                    <label for="checkers" id="chhh" class="icon-link icon-link-2" href="javascript:void(0)">
                                        <input type="checkbox" id="checkers" style="display: none;">
                                        <?php
                                            if (!empty($user_data)) {?>
                                                <i><img style="width: 40px; border-radius: 50%;" src="<?= ASSET.'uploads/profile/' . $user_data->image ?>" alt="" srcset=""></i>
                                                <span class="text">
                                                    <span class="sub">Hi,&nbsp;<?=$user_data->username?> </span>My Account
                                                </span>
                                                <ul class="ulite animate__animated animate__fadeIn animate__fast">
                                                    <li><a href=""><i class="fa fa-home"></i>Dashboard</a></li>
                                                    <li><a href=""><i class="fa fa-cog"></i></i>Account</a></li>
                                                    <li><a href=""><i class="fa fa-comments"></i>Complain</a></li>
                                                    <li><a id="logout" href="javascript:void(0)"><i class="fa fa-sign-out"></i>Logout</a></li>
                                                </ul>
                                        <?php }else {?>
                                            <i class="flaticon-user"></i>
                                            <span class="text">
                                                    <span class="sub">Login </span>My Account
                                            </span>
                                            <ul class="ulite animate__animated animate__fadeIn animate__fast">
                                                <li><a href="<?=ROOT.'login?return='.RETURL?>"><i class="fa fa-sign-in"></i>Login</a></li>
                                                <li><a href="<?=ROOT.'signup?return='.RETURL?>"><i class="fa fa-book"></i></i>Register</a></li>
                                            </ul>
                                        <?php } ?>
                                    </label>
                                                        ...
                                                        <div class="cart__inner d-flex">
                                                            <div class="cart__thumb">
                                                                <a href="product-details.html">
                                                                    <img src="<?=ASSET.FRNTND.'img/cart/20.jpg'?>"
                                                                        alt="">
                                                                </a>
                                                            </div>
